...
prepare_coap_request(oc_client_cb_t *cb)
{
    ...
    transaction = coap_new_transaction(cb->mid, cb->token, cb->token_len, &cb->endpoint);
...
static void finish_download_handler(oc_client_response_t *r)
{
    ...
    if(download_no_error){
        PRINT("Perform upgrade");
        finish_update();
    }
    else
    {
        PRINT("Can't update! Not fully downloaded\n");
    }
...
void download_update_coap(void)
{
    ...
    char enp_string[] = "coap://192.168.1.154";
