...
{ \
    if (receive_cb) \
    { \
        if (usi_twi_amount_data_in_receive_buffer()) \
        { \
            receive_cb(usi_twi_amount_data_in_receive_buffer()); \
        } \
    } \
}
...
static usi_on_request_ptr_t request_cb;
static usi_on_receive_ptr_t receive_cb;
...
void usi_twi_slave_init( uint8_t ownAddress )
{
